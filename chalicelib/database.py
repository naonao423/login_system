...
def check_id_pass(data):
    ...
    if len(items) == 0:
        ...
        if items[0]['password'] == password:
            ...
            error_log = {"error":"not_match"}
            return error_log

def create_session_id(user_id):
    raw_data = user_id + str(arrow.now())
    session_id = hashlib.md5(raw_data.encode("utf-8")).hexdigest()
    table = _get_database().Table(os.environ["DB_TABLE_NAME2"])
    put_part = {}
    put_part['session_id'] = session_id
    put_part['user_id'] = user_id
